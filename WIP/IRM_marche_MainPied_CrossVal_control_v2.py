...
n_p=1000
result_cv_tr_foot,permutation_scores_tr_foot, p_foot=permutation_score(pipeline,roi_foot_all,roi_hand_all,y_foot_all,groups,logo,n_p, random=False) 
print('Train FOOT - IMAG VS STIM',np.array(result_cv_tr_foot).mean(),p_foot)
result_cv_tr_hand,permutation_scores_tr_hand, p_hand=permutation_score(pipeline,roi_hand_all,roi_foot_all,y_hand_all,groups,logo,n_p, random=False)
print('Train HAND - IMAG VS STIM',np.array(result_cv_tr_hand).mean(),p_hand)
#result_cv_tr_imag,permutation_scores_tr_imag, p_imag=permutation_score(pipeline,roi_imag_all,roi_stim_all,y_imag_all,groups,logo,n_p, random=False)   
#print('Train IMAG - HAND VS FOOT',np.array(result_cv_tr_imag).mean(),p_imag)
#result_cv_tr_stim,permutation_scores_tr_stim, p_stim=permutation_score(pipeline,roi_stim_all,roi_imag_all,y_stim_all,groups,logo,n_p, random=False)
...
#
#result_foot,permutation_scores_foot, p_foot=permutation_score(pipeline,roi_foot_all,roi_foot_all,y_foot_all,groups,logo,n_p, random=False) 
#print('FOOT - IMAG VS STIM',np.array(result_foot).mean(),p_foot)
#result_hand,permutation_scores_hand, p_hand=permutation_score(pipeline,roi_hand_all,roi_hand_all,y_hand_all,groups,logo,n_p, random=False)
#print('HAND - IMAG VS STIM',np.array(result_hand).mean(),p_hand)
#result_imag,permutation_scores_imag, p_imag=permutation_score(pipeline,roi_imag_all,roi_imag_all,y_imag_all,groups,logo,n_p, random=False)   
#print('IMAG - HAND VS FOOT',np.array(result_imag).mean(),p_imag)
#result_stim,permutation_scores_stim, p_stim=permutation_score(pipeline,roi_stim_all,roi_stim_all,y_stim_all,groups,logo,n_p, random=False)
